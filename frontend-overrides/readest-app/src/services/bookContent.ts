...
export async function resolveBookContentSource(
  ...
  if (isWebAppPlatform() && book.url?.startsWith('/api/books/')) {
    const path = typeof window === 'undefined' ? book.url : new URL(book.url, window.location.origin).toString();
    return { kind: 'url', path, base: 'None' };
  }
