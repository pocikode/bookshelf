...
export const isReadestCloudEnabled = (settings: SystemSettings | null | undefined): boolean =>
  // Readest Cloud does not exist in the personal deployment — this server is the
  // only backend, and the library syncs through `/api` in `app/library/page.tsx`.
  // Without this the default above turns Cloud on (no third-party provider is
  // configured), so the library pull hit `fetchWithAuth`, found no Supabase JWT
  // and logged "Not authenticated" from `pullChanges` on every load.
  process.env['NEXT_PUBLIC_PERSONAL_APP'] === 'true'
    ? false
