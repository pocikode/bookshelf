...
beforeEach(() => {
  ...
  // Reset window.location. `assign` is part of the stub because leaving the
  // library for the reader is a document navigation, not a router push.
...
describe('navigateToReader', () => {
  ...
  test('leaves the library for /reader/id with a document navigation on web (non-PWA)', () => {
    ...
    // Routing this hop through the App Router is what wedges the library on
    // Back: `/reader/[ids]` is a Pages Router route, so the push can only end
    // in a document navigation anyway, and Firefox restores the library from
    // the back-forward cache with the router still mid-navigation.
    expect(window.location.assign).toHaveBeenCalledWith('/reader/book1');
    expect(router.push).not.toHaveBeenCalled();
  });

  test('switching books from inside the reader stays a client-side push', () => {
    vi.mocked(isWebAppPlatform).mockReturnValue(true);
    vi.mocked(isPWA).mockReturnValue(false);
    Object.defineProperty(window, 'location', {
      value: { pathname: '/reader/book1', search: '', assign: vi.fn() },
      writable: true,
    });

    const router = mockRouter();
    navigateToReader(router, ['book2']);

    expect(router.push).toHaveBeenCalledWith('/reader/book2', undefined);
  ...
  test('appends additional query params for web platform', () => {
    ...
    expect(window.location.assign).toHaveBeenCalledWith('/reader/book1?view=scroll');
  });
