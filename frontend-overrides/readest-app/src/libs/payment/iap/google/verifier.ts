...
export class GoogleIAPVerifier {
  ...
  constructor() {
    ...
    this.androidPublisher = androidpublisher({
      version: 'v3',
      // The generated Android publisher client and the direct auth dependency
      // can resolve different GoogleAuth minor versions in Bun's workspace.
      // Both expose the same runtime auth contract here.
      auth: this.auth as unknown as androidpublisher_v3.Options['auth'],
    });
  }
