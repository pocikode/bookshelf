...
Bookshelf is a self-hosted EPUB/PDF library with user accounts. Reading position is stored per user on the server, so a book resumes at the same EPUB CFI or PDF page on another device.
...
available at `GET /api/version`, in the login and application chrome, and from
...
The first startup seeds the administrator password from `APP_PASSWORD`; afterward the Settings page can change it. Changing the administrator password invalidates every session. Administrators can create users, reset their passwords, and disable access. Each user can revoke their own current session or all of their sessions.

Books uploaded by an administrator are public by default. Users can upload books as public or private; private books are visible only to their owner and administrators. Reading progress is private to each user.
...
- Passwords are stored as SHA-256 digests and compared in constant time; new user passwords must be at least 8 Unicode code points.
